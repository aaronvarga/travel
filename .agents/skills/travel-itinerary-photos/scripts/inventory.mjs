...
// --- the home-page card source of truth ---
let card = { src: null, host: null, alt: null };
try {
  const entry = require('../../../../src/_data/card-images.js')[slug];
  if (entry) card = { src: entry.path, host: classify(entry.path), alt: entry.alt };
} catch { /* card manifest absent — verify will report it */ }
...
console.log(`   home-page card (card-images manifest): ${card.src ? card.host + '  ' + card.src.slice(0, 60) : 'not found'}`);
