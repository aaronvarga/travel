/**
 * One-sentence promises for the decision-hub cards.
 *
 * Keep these experiential and destination-specific. The score, budget, PTO,
 * evidence, and readiness already have dedicated UI elsewhere on each card.
 */
module.exports = {
  albania: 'Combines five nights in the Accursed Mountains with historic towns and a long Ksamil finish for clear, warm Ionian swimming.',
  balkans: 'Road-trips from Mostar and the Bay of Kotor to Albania and Corfu, balancing dramatic old towns, mountain scenery, and Adriatic swims.',
  'california-pacific-coast': 'Links San Diego, Santa Barbara, Monterey, and San Francisco with beaches, wildlife, coastal hikes, and the classic Big Sur drive.',
  'canary-islands': 'Pairs Tenerife and La Palma for volcanic hiking, black-sand beaches, whale watching, and lush island scenery without a mainland transfer.',
  croatia: 'Moves slowly through Split, Hvar, and Korčula for stone old towns, boat days, clear Adriatic coves, and an easy island-focused rhythm.',
  'switzerland-crete': 'Splits five nights of Alps — Jungfraujoch, the Grindelwald-First Cliff Walk, Lauterbrunnen’s waterfalls — with seven nights of warm Aegean swimming on Crete’s Chania coast.',
  'switzerland-sicily': 'Pairs five nights of Alps — Jungfraujoch, the Grindelwald-First Cliff Walk, Lauterbrunnen’s waterfalls — with seven warm-water nights in Sicily: Taormina’s Isola Bella, Mount Etna, Cefalù, and baroque Ortigia.',
  'slovenia-adriatic': 'Drives Lake Bled, the Vršič Pass, and the emerald Soča Valley into a coastal Piran finish — alpine scenery, caves, and cool Adriatic swimming within one country.',
  'dolomites-sardinia': 'Combines Venice and high-alpine Dolomites hiking with a warm-water Sardinia finish, delivering the trip’s strongest mountain-to-beach contrast.',
  'greece-via-lisbon': 'Uses Lisbon and Athens as short cultural bookends around a Crete-focused trip of Venetian towns, mountain drives, and standout beaches.',
  'greece-ionian': 'Links road-connected Lefkada with Kefalonia for warm, calm turquoise swimming — Porto Katsiki’s cliffs, Myrtos, Melissani’s blue cave lake, Assos, and a Navagio boat day — with minimal ferry stress.',
  'greece-cyclades': 'Bookends a short Athens culture opener with three iconic Cyclades — Naxos’s sandy family beaches and mountain villages, Paros’s Naoussa harbor and Kolymbithres coves, and Milos’s Sarakiniko moonscape and Kleftiko sea caves.',
  hawaii: 'Splits time between Oʻahu and Hawaiʻi Island for warm swimming, snorkeling, volcanoes, waterfalls, and an easy domestic two-island adventure.',
  'maui-kauai': 'Pairs Maui’s warm beaches, Haleakalā, and Hāna with Kauai’s Nā Pali Coast, Waimea Canyon, Poʻipū swimming, and a slow Hanalei finish.',
  iceland: 'Builds a photography-first loop around Reykjavík, waterfalls, black-sand beaches, glacier lagoons, geothermal pools, and immense volcanic landscapes.',
  'iceland-ischia-cilento': 'Pairs four Iceland nights of waterfalls, black sand, and geothermal water with seven Ischia/Cilento nights and a Rome-airport buffer; the flight construction still requires a reroute.',
  'italy-salento-amalfi': 'Connects Gallipoli, Maratea, and Sorrento for Italian food, historic coastal towns, beach time, and the cinematic cliffs of the Amalfi Coast.',
  'madeira-crete': 'Pairs Madeira’s dramatic levada and ridge hiking with Crete’s warm beaches, Venetian towns, and family-friendly Mediterranean swimming.',
  'madeira-kefalonia': 'Pairs Madeira’s levada walks, cloud-high ridge and volcanic coast with Kefalonia’s Myrtos cliffs, blue caves, calm swims, harbor towns, and Athens-protected return.',
  'madeira-mallorca': 'Balances Madeira’s cloud-high trails and volcanic coast with Mallorca’s sandy coves, mountain villages, and warmer, easier swimming.',
  'madeira-sicily': 'Combines Madeira’s hiking and lush scenery with Cefalù and eastern Sicily for beaches, Italian food, historic towns, and Mount Etna.',
  portugal: 'Offers the best-value mix of Cascais, Algarve coves, and Madeira hiking with fewer expensive transfers and a relaxed three-base route.',
  'portugal-algarve-sicily': 'Moves from Lisbon and the Algarve to Sicily for golden coves, dramatic towns, Mount Etna, and Italian food, with more transfers than the simpler hybrids.',
  'portugal-crete': 'Blends Portugal’s cities and Atlantic coast with Crete’s warmer water, beaches, historic towns, and a balanced hiking-and-swimming finish.',
  crete: 'Crosses Crete from Chania’s lagoons and Imbros Gorge through Venetian Rethymno to Knossos, Spinalonga, and the clear water of Mirabello Bay.',
  'portugal-sicily': 'Pairs Portugal’s coast and city time with Cefalù, Taormina, and Mount Etna for a scenery-rich mix of beaches, towns, and Italian food.',
  'sardinia-corsica': 'Centers the trip on exceptional Mediterranean swimming, pairing Sardinia’s turquoise coves with Bonifacio’s cliffs, boat days, and compact old towns.',
  'sicily-malta': 'Combines Sicily’s food, beaches, Mount Etna, and historic towns with Malta’s limestone cities, boat trips, and brilliant blue swimming.',
  'southern-france': 'Moves from Nice to Cassis and Aix for Riviera beaches, Calanques hiking, Provençal markets, polished towns, and a comfortable three-base pace.',
  spain: 'Links Seville, Granada, and the Costa del Sol for landmark architecture, tapas, white villages, mountain scenery, and a relaxed beach finish.',
  'turkish-riviera': 'Follows Antalya, Kaş, and Fethiye for warm turquoise water, boat days, ancient ruins, dramatic coastal hikes, and strong overall value.',
};
